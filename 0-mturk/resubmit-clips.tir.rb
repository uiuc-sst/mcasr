...
# Copied from restitch-clips-tir.rb.

require 'csv'
$mp3s = []
CSV.foreach('/r/lorelei/aug/to-mturk-tigrinya.csv') {|r|
...
CSV.foreach('/r/lorelei/aug/bat/tir/all.csv') {|r|
  ...
  wavs = mp3s.map {|l| l.sub("http://isle.illinois.edu/mc/tigrinya/IL5_EVAL_", "") .sub(".mp3", "") } \
...
$mp3s.each {|mp3|
  ...
  # puts "#{mp3} -> #{t}" if again
  # http://isle.illinois.edu/mc/tigrinya/IL5_EVAL_006_003-47349876-48595924.mp3 -> ["[music]", "doododoodooo", "[music]"]
  $agains << mp3[0..-5] if again # Strip .mp3.
}
STDERR.puts "Analyzed transcriptions."
$agains = $agains.shuffle

`rm -rf newclips-tigrinya.csv`
File.open("newclips-tigrinya.csv", "w") {|f|
...
STDERR.puts "Fresh batch is in newclips-tigrinya.csv."
