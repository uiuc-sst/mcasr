...
# Stitch turkers' transcriptions for Russian 2017-07-12.
# Report timings in microseconds.  SR = 22050 Hz, if that matters.

# Clip: "http://isle.illinois.edu/mc/2017-07-12-rus/RUS_103_019-96094050-97342024.mp3"
...
# Each line: the "RUS_103_019-96094050-97342024.mp3"'s, then the last 8 column-delimited "strings".
# Indices are documented in preprocess_turker_transcripts.pl.
require 'csv'
CSV.foreach('/r/lorelei/russian/batchBoth.csv') {|r|
  ...
  wavs = r[27..42].select {|c| c =~ /mp3/} \
    .map {|l| l.sub("http://isle.illinois.edu/mc/2017-07-12-rus/RUS_", "") .sub(".mp3", "") } \
...
# Collect each RUS_999_999 utterance's clips+transcriptions.
$u = Hash.new {|k,v| k[v] = []}
$t.each {|wav,t|
...
#$u.each {|utt,clips|
#  p utt
#  clips.to_a.sort_by {|x| x[0]} each {|clip| p clip}
#}

`rm -rf clips-russian.tar clips-russian; mkdir clips-russian`
$u.each {|utt,clips|
  File.open("clips-russian/" + utt + ".txt", "w") {|f|
    clips.to_a.sort_by {|x| x[0]} \
      .each {|c| f.puts "#{c[0]} #{c[1]} #{c[2].sort.uniq.join(' # ')}" }
  }
}
`tar cf clips-russian.tar clips-russian`
