...
tmpdir=$dir
gunzip -c $lat | lattice-to-fst --lm-scale=$lm_scale --rm_eps=false --acoustic-scale=$acoustic_scale ark:- "scp,p:echo $uttid $tmpdir/$uttid.fst|" || exit 1;
#gunzip -c $lat | lattice-to-fst --lm-scale=$lm_scale --acoustic-scale=$acoustic_scale ark:- "ark:$tmpdir/lat.fst"
: <<'END'
! [ -s $tmpdir/$uttid.fst ] && \
