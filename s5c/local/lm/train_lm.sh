...
# LM order, prefer a lower order model 
# Since we are not decoding with this
model_order=2
...
if [ "$stage" -le 1 ]; then
  ...
  find $corpus_dir -mindepth 1 -maxdepth 1 -type f |\
...
#$(find $norm_dir -iname '*.txt')
  for f in $(find $corpus_dir -iname '*.txt'); do cat $f; done |\
...
echo "LM order: $model_order"

unigram_lm=$lm_dir/lm_unigram.arpa.gz
if [ "$stage" -le 4 ]; then
  echo "Training a Unigram LM ..."
  command -v ngram-count 1>/dev/null 2>&1 || { echo "Please install SRILM and set path.sh accordingly"; exit 1; }
  ngram-count -order 1  -wbdiscount -interpolate \
    -unk -map-unk "<UNK>" -limit-vocab -vocab $vocab -text $full_corpus -lm $unigram_lm || exit 1
  du -h $unigram_lm
fi

if [ $model_order -lt 2 ] ; then exit 0; fi

bigram_lm=$lm_dir/lm_bigram.arpa.gz

if [ "$stage" -le 4 ]; then
  echo "Training a Bigram LM ..."
  command -v ngram-count 1>/dev/null 2>&1 || { echo "Please install SRILM and set path.sh accordingly"; exit 1; }
  ngram-count -order 2  -wbdiscount -interpolate \
    -unk -map-unk "<UNK>" -limit-vocab -vocab $vocab -text $full_corpus -lm $bigram_lm || exit 1
  du -h $bigram_lm
fi


if [ $model_order -lt 3 ] ; then exit 0; fi
...
if [ "$stage" -le 4 ]; then
  ...
  ngram-count -order 3  -wbdiscount -interpolate \
...
if [ $model_order -lt 4 ] ; then exit 0; fi
