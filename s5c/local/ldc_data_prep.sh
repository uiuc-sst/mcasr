...
[ -d $data/$lang_subdir ] || { echo "Missing data/lang_subdir directory of .flac files '$data/$lang_subdir'. Check the settings file."; exit 1; }

dst=data/$lang
mkdir -p $dst
find $data/$lang_subdir -type f -name \*.flac > $dst/all.flist
wavdir=$dst/wav
mkdir -p $wavdir
